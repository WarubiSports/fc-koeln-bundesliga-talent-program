...
export async function checkLoginAttempts(appId, email) {
...
export async function recordFailedLogin(appId, email) {
  const key = `${appId}:${email.toLowerCase()}`;
...
export async function clearLoginAttempts(appId, email) {
  const key = `${appId}:${email.toLowerCase()}`;
...
export async function checkResetRateLimit(appId, email) {
  const key = `${appId}:${email.toLowerCase()}`;
  ...
  try {
    ...
    if (record.attempts >= RESET_RATE_LIMIT) {
      const waitMinutes = Math.ceil((new Date(record.window_end) - new Date()) / 60000);
...
export async function recordResetAttempt(appId, email) {
  const key = `${appId}:${email.toLowerCase()}`;
