...
describe('Grocery E2E - Full Middleware Stack', () => {
  const TEST_APP_ID = 'fckoln';
  const PLAYER_EMAIL = 'e2e-grocery-player@fckoln.com';
  const STAFF_EMAIL = 'e2e-grocery-staff@fckoln.com';
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await pool.query(
        ...
        [playerUserId, TEST_APP_ID, PLAYER_EMAIL, hashedPassword, 'E2E', 'Player', 'player', 'active', 'true']
      );
      ...
      await pool.query(
        `INSERT INTO users (id, app_id, email, password, first_name, last_name, role, status, approved)
         ...
         ON CONFLICT (email, app_id) DO UPDATE SET 
           id = EXCLUDED.id,
           password = EXCLUDED.password`,
        [staffUserId, TEST_APP_ID, STAFF_EMAIL, hashedPassword, 'E2E', 'Staff', 'staff', 'active', 'true']
      );
      ...
      const itemResult = await pool.query(
        `INSERT INTO grocery_items (app_id, name, category, price)
         VALUES ($1, $2, $3, $4)
         RETURNING id`,
        [TEST_APP_ID, 'E2E Test Apple', 'fruits', '2.50']
      );
      testItemId = itemResult.rows[0].id;
    } catch (error) {
      console.error('E2E Grocery test setup failed:', error);
    }
  });
  ...
  afterAll(async () => {
    try {
      ...
      console.error('E2E Grocery test cleanup failed:', error);
    }
  });

  describe('GET /api/fckoln/grocery/items', () => {
    it('should return 401 without auth token', async () => {
      const response = await request(app)
        .get('/api/fckoln/grocery/items');

      expect(response.status).toBe(401);
    });

    it('should return items for authenticated player', async () => {
      const response = await request(app)
        .get('/api/fckoln/grocery/items')
    ...
    it('should filter items by category', async () => {
      const response = await request(app)
        .get('/api/fckoln/grocery/items?category=fruits')
        .set('Authorization', `Bearer ${playerToken}`);

      expect(response.status).toBe(200);
      response.body.items.forEach((item: { category: string }) => {
        expect(item.category).toBe('fruits');
      });
    });
  });

  describe('POST /api/fckoln/grocery/orders', () => {
    it('should create order within budget', async () => {
      const deliveryDate = new Date(Date.now() + 86400000).toISOString().split('T')[0];
      
      const response = await request(app)
        .post('/api/fckoln/grocery/orders')
        .set('Authorization', `Bearer ${playerToken}`)
        .send({
          deliveryDate,
      ...
      expect(parseFloat(response.body.totalAmount)).toBe(5.00);

      if (response.body.orderId) {
        await pool.query('DELETE FROM grocery_order_items WHERE order_id = $1', [response.body.orderId]);
        await pool.query('DELETE FROM grocery_player_orders WHERE id = $1', [response.body.orderId]);
      }
    });
    ...
    it('should reject order exceeding budget', async () => {
      ...
      expect(response.body.message).toMatch(/exceeds|budget/i);
    });
    ...
    it('should return 400 for invalid item', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.body.message).toMatch(/invalid|unavailable|not found/i);
    });
  });

  describe('GET /api/fckoln/grocery/orders', () => {
    ...
    it('should return orders for authenticated player', async () => {
      const response = await request(app)
        .get('/api/fckoln/grocery/orders')
        .set('Authorization', `Bearer ${playerToken}`);
    ...
    it('should return orders for authenticated staff', async () => {
      const response = await request(app)
        .get('/api/fckoln/grocery/orders')
  ...
  describe('Access Control', () => {
    it('should prevent player from creating grocery items', async () => {
      const response = await request(app)
        .post('/api/fckoln/admin/grocery/items')
        .set('Authorization', `Bearer ${playerToken}`)
    ...
    it('should allow staff to create grocery items', async () => {
      const response = await request(app)
        .post('/api/fckoln/admin/grocery/items')
        .set('Authorization', `Bearer ${staffToken}`)
        .send({ name: 'Staff E2E Item', category: 'fruits', price: '5.00' });
