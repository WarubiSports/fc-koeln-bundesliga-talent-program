...
// FC Köln Management System HTML (embedded)
const fcKolnHTML = `<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="login-card">
        ...
        <div class="status">
            <span style="color: #28a745;">●</span> System Online | Deployment Ready
        </div>
        ...
        function login(event) {
            ...
            if (password === 'ITP2024') {
                if (email === 'max.bisinger@warubi-sports.com') {
                    ...
                    localStorage.setItem('auth-token', 'admin_' + Date.now());
                    ...
                    localStorage.setItem('auth-token', 'staff_' + Date.now());
                    localStorage.setItem('user', JSON.stringify({
                    ...
                    messageDiv.innerHTML = '<div class="message" style="background: #f8d7da; color: #721c24;">Email not recognized. Please use provided credentials.</div>';
                }
            } else {
                messageDiv.innerHTML = '<div class="message" style="background: #f8d7da; color: #721c24;">Invalid password. Please try again.</div>';
            }
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'healthy', system: 'FC Köln Management' });
});
  ...
  if (password === 'ITP2024') {
    if (email === 'max.bisinger@warubi-sports.com') {
      res.json({
        ...
        user: { name: 'Max Bisinger', email, role: 'admin' }
      });
    } else if (email === 'thomas.ellinger@warubi-sports.com') {
      res.json({
        message: 'Login successful',
        token: 'staff_' + Date.now(),
        user: { name: 'Thomas Ellinger', email, role: 'staff' }
      });
    } else {
...
// Serve FC Köln interface for all routes
app.get('*', (req, res) => {
  res.send(fcKolnHTML);
});
