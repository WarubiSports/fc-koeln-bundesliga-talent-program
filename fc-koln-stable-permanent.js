...
<body>
    ...
    <div class="main-app" id="mainApp">
        ...
        <div class="nav-tabs">
            ...
            <button class="nav-tab admin-only" data-page="order-overview">Order Overview</button>
    ...
    <script>
        ...
        function initializeOrderOverview() {
            ...
            if (exportAllBtn) {
                exportAllBtn.addEventListener('click', exportAllOrdersCSV);
            }
            
            if (generateShoppingListBtn) {
                generateShoppingListBtn.addEventListener('click', generateConsolidatedShoppingList);
            }
        }
        
        function loadHouseSummaries() {
            const orderHistory = JSON.parse(localStorage.getItem('fckoln_order_history') || '[]');
            const players = JSON.parse(localStorage.getItem('fckoln_players') || '[]');
            
            // Group orders by player house
            const houseSummaries = {
                'Widdersdorf 1': { orders: 0, totalValue: 0, players: new Set() },
                'Widdersdorf 2': { orders: 0, totalValue: 0, players: new Set() },
                'Widdersdorf 3': { orders: 0, totalValue: 0, players: new Set() }
            };
            
            orderHistory.forEach(order => {
                const player = players.find(p => p.id === order.playerId);
                const house = player ? player.house : 'Unknown';
                
                if (houseSummaries[house]) {
                    houseSummaries[house].orders++;
                    houseSummaries[house].totalValue += order.total;
                    houseSummaries[house].players.add(order.playerName);
                }
            });
            
            // Update UI
            updateHouseSummaryCard('w1', houseSummaries['Widdersdorf 1']);
            updateHouseSummaryCard('w2', houseSummaries['Widdersdorf 2']);
            updateHouseSummaryCard('w3', houseSummaries['Widdersdorf 3']);
        }
        
        function updateHouseSummaryCard(houseId, summary) {
            const activeCountEl = document.getElementById(houseId + 'ActiveCount');
            const totalValueEl = document.getElementById(houseId + 'TotalValue');
            const playersCountEl = document.getElementById(houseId + 'PlayersCount');
            
            if (activeCountEl) activeCountEl.textContent = summary.orders;
            if (totalValueEl) totalValueEl.textContent = '€' + summary.totalValue.toFixed(2);
            if (playersCountEl) playersCountEl.textContent = summary.players.size;
        }
        ...
        function loadDetailedOrders() {
            ...
            if (orderHistory.length === 0) {
                container.innerHTML = '<p class="no-orders">No orders have been placed yet.</p>';
                return;
            }
        ...
        function createOrderCard(order) {
            ...
            Object.keys(order.items).forEach(itemId => {
                ...
                if (itemData && item.selected && item.quantity > 0) {
                    const itemTotal = itemData.price * item.quantity;
                    itemsHtml += '<div class="order-item">' +
                        '<span class="order-item-name">' + itemData.name + '</span>' +
            ...
            return '<div class="order-card">' +
                '<div class="order-header">' +
                    ...
                    '</div>' +
                    '<div class="order-info">' +
                        '<div class="order-info-label">Player</div>' +
                        '<div class="order-info-value">' + order.playerName + '</div>' +
                    '</div>' +
                    '<div class="order-info">' +
        ...
        function applyOrderFilters() {
            ...
            // In a real implementation, you would filter the displayed orders
            loadDetailedOrders();
        }
        ...
        function generateConsolidatedShoppingList() {
            ...
            orderHistory.forEach(order => {
                Object.keys(order.items).forEach(itemId => {
                    ...
                    if (itemData && item.selected && item.quantity > 0) {
                        ...
                        consolidatedItems[itemId].totalQuantity += item.quantity;
                        consolidatedItems[itemId].orders.push(order.orderNumber);
                        
                        // Get player house
                        const players = JSON.parse(localStorage.getItem('fckoln_players') || '[]');
                        const player = players.find(p => p.id === order.playerId);
                        if (player && player.house) {
                            consolidatedItems[itemId].houses.add(player.house);
                        }
                    }
                });
            });
            ...
            const container = document.getElementById('consolidatedShoppingList');
            if (!container) return;
            
            if (Object.keys(consolidatedItems).length === 0) {
                container.innerHTML = '<p class="no-items">No orders to consolidate.</p>';
                return;
            }
            
            let html = '<div class="consolidated-item" style="background: #dc143c; color: white; font-weight: bold;">' +
                '<div>Item Name</div>' +
                '<div>Total Qty</div>' +
                '<div>Houses</div>' +
                '<div>Total Cost</div>' +
            ...
            let grandTotal = 0;
            Object.keys(consolidatedItems).forEach(itemId => {
                const item = consolidatedItems[itemId];
                const totalCost = item.price * item.totalQuantity;
                grandTotal += totalCost;
                
                const housesText = Array.from(item.houses).join(', ') || 'Unknown';
                
                html += '<div class="consolidated-item">' +
                    '<div class="consolidated-item-name">' + item.name + '</div>' +
                    '<div class="consolidated-item-qty">' + item.totalQuantity + '</div>' +
                    '<div class="consolidated-item-houses">' + housesText + '</div>' +
                    '<div class="consolidated-item-total">€' + totalCost.toFixed(2) + '</div>' +
                '</div>';
            });
            
            html += '<div class="consolidated-item" style="background: #f3f4f6; font-weight: bold; border: 2px solid #dc143c;">' +
                '<div>TOTAL</div>' +
                '<div>-</div>' +
                '<div>-</div>' +
        ...
        function exportAllOrdersCSV() {
            ...
            let csvContent = 'Order Number,Player Name,Order Date,Delivery Date,Item Name,Quantity,Unit Price,Total Price,Order Total\\n';
            ...
            orderHistory.forEach(order => {
                ...
                Object.keys(order.items).forEach(itemId => {
                    ...
                    if (itemData && item.selected && item.quantity > 0) {
                        ...
                        csvContent += '"' + order.orderNumber + '","' + order.playerName + '","' + 
                                     orderDate + '","' + order.deliveryDate + '","' + itemData.name + '",' + 
                                     item.quantity + ',' + itemData.price.toFixed(2) + ',' + 
                                     itemTotal.toFixed(2) + ',' + order.total.toFixed(2) + '\\n';
                    }
                });
            });
            ...
            const url = URL.createObjectURL(blob);
            link.setAttribute('href', url);
            link.setAttribute('download', 'fc_koln_all_orders_' + new Date().toISOString().split('T')[0] + '.csv');
            link.style.visibility = 'hidden';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
